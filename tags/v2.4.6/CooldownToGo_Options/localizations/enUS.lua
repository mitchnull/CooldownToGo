-- Generated by Babelfish script, do not add strings manually, only translate existing strings.
-- This is the base locale; values can be "true" so they default to their key, or any string to override that behaviour.
local L = LibStub("AceLocale-3.0"):NewLocale("CooldownToGo_Options", "enUS", true)


L["Bottom"] = "Bottom"
L["Color"] = "Color"
L["Delay before cooldown display is activated (useful for button-smashers)"] = "Delay before cooldown display is activated (useful for button-smashers)"
L["Disable showing the cooldown near the expiry time without user action"] = "Disable showing the cooldown near the expiry time without user action"
L["Fade time"] = "Fade time"
L["Fade time of the message in seconds"] = "Fade time of the message in seconds"
L["Font"] = "Font"
L["Font outline"] = "Font outline"
L["Font size"] = "Font size"
L["Frame strata"] = "Frame strata"
L["Grace Period"] = "Grace Period"
L["High"] = "High"
L["Hold time"] = "Hold time"
L["Icon size"] = "Icon size"
L["If checked, then only trigger for actions that are on the ignore list, and ignore the others."] = "If checked, then only trigger for actions that are on the ignore list, and ignore the others."
L["Ignore list"] = "Ignore list"
L["Ignore next action"] = "Ignore next action"
L["Items"] = "Items"
L["Left"] = "Left"
L["Lock/Unlock display frame"] = "Lock/Unlock display frame"
L["Locked"] = "Locked"
L["Low"] = "Low"
L["Medium"] = "Medium"
L["None"] = "None"
L["Normal"] = "Normal"
L["Padding"] = "Padding"
L["Petbar"] = "Petbar"
L["Play a sound when the cooldown reaches Ready Time"] = "Play a sound when the cooldown reaches Ready Time"
L["Ready time"] = "Ready time"
L["Reverse Ignore Logic"] = "Reverse Ignore Logic"
L["Right"] = "Right"
L["Show the cooldown again this many seconds before the cooldown expires"] = "Show the cooldown again this many seconds before the cooldown expires"
L["Spells"] = "Spells"
L["Strata"] = "Strata"
L["Suppress Ready Notification"] = "Suppress Ready Notification"
L["Text position"] = "Text position"
L["Thick"] = "Thick"
L["Time to hold the message in seconds"] = "Time to hold the message in seconds"
L["Top"] = "Top"
L["Warning Sound"] = "Warning Sound"
L["Warning Sound Name"] = "Warning Sound Name"
